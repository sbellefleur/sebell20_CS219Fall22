package Crypto;

public class ceasar_cipher {

    public static String E(String clear, int key){
        String cipher = "";
        final int alpha_length = 26; // final means that this value should never change
        for (int i = 0; i < clear.length(); i++){
            char clear_char = clear.charAt(i);
            int clear_char_pos = clear_char - 'a';
            int cipher_char_pos = Math.floorMod(clear_char_pos + key, alpha_length);
            char cipher_char = (char)(cipher_char_pos + 'a');
            cipher += cipher_char;
        }
        return cipher;
    }
    public static String D(String cipher, int key){
        String plain = "";
        final int alpha_length = 26;
        for (int i = 0; i < cipher.length(); i ++){
            char cipher_char = cipher.charAt(i);
            int cipher_char_pos = cipher_char - 'a';
            int plain_char_pos = Math.floorMod(cipher_char_pos - key, alpha_length);
            char plain_char = (char)(plain_char_pos + 'a');
            plain += plain_char;
        }
        return plain;
    }
    public static String d(String cipher, int key){
        return E(cipher, -key); //this is the same as the decrypt function we just wrote
    }

    public static void main (String [] args){
        System.out.println(E("cold", 20)== "wifx");// not how you compare strings in java
        System.out.println(E("cold", 20).equals("wifx"));//How to properly compare strings in java
        System.out.println(D(E("cold", 20), 20).equals("cold"));
        System.out.println(d(E("cold", 20), 20).equals("cold"));
    }
}
